...
public class CommunityController {
	...
	@ResponseBody	// 데이터로 전송 @ResponseBody
	public String api_data() throws Exception {
		//api 호출
	    String service_key = "eEMWUqlUDcFvHmN6L0%2BV1lKUXC1YpGLuAkhU%2Fmkb%2FwZI4sFmQakEjx21opj117BCIJjBXIpd%2BCq6dpkkT%2B6exA%3D%3D";
	...
	public String api_comment() throws Exception {
		//api 호출
		String service_key = "eEMWUqlUDcFvHmN6L0%2BV1lKUXC1YpGLuAkhU%2Fmkb%2FwZI4sFmQakEjx21opj117BCIJjBXIpd%2BCq6dpkkT%2B6exA%3D";
		...
		urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
		urlBuilder.append("&" + URLEncoder.encode("pageNo","UTF-8") + "=" + URLEncoder.encode(page, "UTF-8")); /*페이지번호*/
		urlBuilder.append("&" + URLEncoder.encode("from","UTF-8") + "=" + URLEncoder.encode("ETC", "UTF-8")); /*OS 구분 : IOS (아이폰), AND (안드로이드), WIN (윈도우폰), ETC(기타)*/
		urlBuilder.append("&" + URLEncoder.encode("until","UTF-8") + "=" + URLEncoder.encode("AppTest", "UTF-8")); /*서비스명(어플명)*/
